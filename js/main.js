...
My.prototype.money = function (value, params) {
  ...
  if (params && params.decimal) {
    ...
    console.log(
      temp.indexOf("."),
      temp.slice(temp.indexOf(".")).length,
      "????"
    );
    if (!temp.includes(".")) {
      console.log(0);
 ...
 * @param {string} field 需要改变值的字段
 * @param {number|string} precision 精确度 2
 * @returns {boolean} 如果输入的值不符合条件则返回false
 */
My.prototype.numberTransform = function (that, field, precision) {
  if (/\d+[.]\d+/.test(that[field])) {
    let reg = new RegExp("\\d+[.]\\d{0," + precision + "}$");

    if (!reg.test(that[field])) {
      that.$nextTick(() => {
        that[field] = that.$temp || that.$temp.slice(0, -1);
      });
      return false;
    }
  }

  if (/[+|-]/.test(that[field]) || !that[field]) {
    setTimeout(() => {
      that[field] = that.$temp || that[field].slice(0, -1);
    });
    return false;
  }

  that.$temp = that[field];
};

/**
 * 获取地址栏数据
 * @returns { object } url 的参数
 */
My.prototype.queryUrlParams = function () {
  // 获取搜索信息url 后面的信息
  const obj = {};
