...
template(slot-scope='scope', v-if="scope.height > 0")
    el-table.table(:data="tableData", @row-click="rowExpand", row-key="id", :expand-row-keys="entexpands", v-loading="loading", align="center", stripe, :height="scope.height")
        .no-data(slot="empty", :style!="{height: (scope.height - 80) + 'px'}")
